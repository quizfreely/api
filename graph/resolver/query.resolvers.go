...
func (r *queryResolver) Term(ctx context.Context, id string) (*model.Term, error) {
	...
	if authedUser != nil {
		err = pgxscan.Get(
			...
			r.DB,
			&term,
			`SELECT terms.id, terms.term, terms.def, ($3||t.term_image_key) as term_image_url, ($3||t.def_image_key) as def_image_url, terms.sort_order,
...
WHERE terms.id = $1 AND (
		...
		err = pgxscan.Get(
			...
			r.DB,
			&term,
			`SELECT terms.id, terms.term, terms.def, ($2||t.term_image_key) as term_image_url, ($2||t.def_image_key) as def_image_url, terms.sort_order,
		to_char(terms.created_at, 'YYYY-MM-DD"T"HH24:MI:SS.MSTZH:TZM') as created_at,
		to_char(terms.updated_at, 'YYYY-MM-DD"T"HH24:MI:SS.MSTZH:TZM') as updated_at
FROM terms
JOIN studysets ON terms.studyset_id = studysets.id
